...
## 12. COMUNICADO aos agentes (2026-08-24) — Portabilidade multiplataforma INTEGRADA

> De: **agente Linux (ox-alpha)** · Para: agentes macOS/Windows e próximas sessões.

**Situação:** dois agentes trabalharam em paralelo na portabilidade (Linux e
Windows). O trabalho do Windows (`docs/MULTIPLATAFORMA.md`, commit 9776371) foi
integrado ao do Linux (`docs/AUDITORIA-LINUX.md`). Estado final consolidado:

1. **`core/Cargo.toml`** — `ort` por plataforma: `coreml` (macOS),
   `directml` (Windows), CPU (Linux). `trash = "5"` para lixeira nativa.
   **Não re-adicionar features globais de EP** (quebra build multiplataforma).
2. **`core/src/ml.rs`** — `models_dir()` em runtime:
   `OPENSHOOT_MODELS_DIR` → diretório dev → busca subindo a partir do exe.
   **O main process define a env var** antes do `loadCore()` (essencial no app
   empacotado: `app.asar.unpacked/core/models` NÃO é achado pelo walk do exe,
   pois Resources não é ancestral de Contents/MacOS).
3. **`core/src/lib.rs`** — cache via `dirs::cache_dir()`; lixeira via
   `trash::delete` nas 3 plataformas (decisão manual-trash macOS superada —
   ver ROADMAP "Registro de decisões").
4. **`electron-builder.yml`** — NSIS (win), AppImage+deb (linux, com .desktop
   e depends); `asarUnpack`: `core/**/*.node` + `core/models/**`.
5. **CI** — matriz macos/ubuntu/windows para `cargo test` com deps Linux
   (pkg-config, libssl-dev, libgomp1); typecheck; clippy ubuntu.

**Validação pós-integração:** 60/60 testes Rust no macOS host + Ubuntu 24.04
container; typecheck limpo; smoke do addon no Windows (agente Windows).

**Limitação conhecida Linux:** ONNX Runtime pré-compilado exige glibc ≥ 2.38 e
CPU com AVX2. Em CPUs antigas (sem AVX2) a inferência ML dá SIGILL — o app
degrada graciosamente para heurística. Alternativas em
`docs/AUDITORIA-LINUX.md §3`.
