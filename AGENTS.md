...
Inferência: CoreML no macOS, DirectML no Windows, CPU ONNX Runtime no Linux
(ver `docs/AUDITORIA-LINUX.md` e `docs/MULTIPLATAFORMA.md`). Os modelos ONNX
são resolvidos em runtime pela env var `OPENSHOOT_MODELS_DIR` (definida pelo
main process) + busca a partir do executável. Lixeira nativa multiplataforma
via crate `trash`.
...
npm run smoke:core    # carrega o addon .node sem Electron (valida build nativo)
...
Caminhos de cache são multiplataforma (`dirs::cache_dir()`).
...
reconhecimento facial) + **portabilidade multiplataforma concluída**
(2026-08-24: Linux por agente Linux — `docs/AUDITORIA-LINUX.md`; Windows por
agente Windows — `docs/MULTIPLATAFORMA.md`). Pendências priorizadas em
`docs/ROADMAP.md`: P2 olhos fechados no culling; release workflow; CUDA opt-in.
