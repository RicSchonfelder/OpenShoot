...
| **CodeFormer** (e.g. `codeformer-v0.1.0.pth`) | Optional local face restoration (opt-in CLI bridge, `docs/CODEFORMER.md`) | [sczhou/CodeFormer](https://github.com/sczhou/CodeFormer) | **NTU S-Lab License 1.0** — non-commercial research use; commercial use requires a separate agreement. ⚠️ **Not bundled, not downloaded, not distributed by OpenShoot**: the user provides the bridge executable and the weights locally and is responsible for license compliance. |
| **facexlib / RetinaFace** (e.g. `RetinaFace-Resnet50.pth`) | Face detection used by the optional CodeFormer bridge | [xinntao/facexlib](https://github.com/xinntao/facexlib) | Apache-2.0 (code); weight files follow their distribution terms. User-provided, not distributed. |
| **Real-ESRGAN** (e.g. `RealESRGAN_x2plus.pth`) | Optional background enhancement in the CodeFormer bridge | [xinntao/Real-ESRGAN](https://github.com/xinntao/Real-ESRGAN) | BSD-3-Clause (code); weights released under the project's license. User-provided, not distributed. |
| **BasicSR** | Training framework dependency of the optional CodeFormer bridge | [XPixelGroup/BasicSR](https://github.com/XPixelGroup/BasicSR) | Apache-2.0. Used only inside the user's own bridge environment; not distributed. |
